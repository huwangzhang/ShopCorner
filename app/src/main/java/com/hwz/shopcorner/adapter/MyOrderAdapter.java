...
import com.hwz.shopcorner.bean.OrderItem;
...
public class MyOrderAdapter extends SimpleAdapter<Order> {
    ...
    public void bindData(BaseViewHolder viewHoder, final Order item) {
        ...
        List<OrderItem> orderItems = item.getItems();
        if (orderItems != null) {
            for (OrderItem imageDetail : orderItems) {
                ImageInfo info = new ImageInfo();
                info.setThumbnailUrl(imageDetail.getWares().getImgUrl());
                info.setBigImageUrl(imageDetail.getWares().getImgUrl());
                imageInfo.add(info);
            }
        }

        NineGridView nineGridView = viewHoder.getNineGridView(R.id.iv_ngrid_layout);
